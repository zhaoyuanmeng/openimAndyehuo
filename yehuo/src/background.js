...
// 悬浮窗相关变量
let notificationWindow = null; // 消息列表悬浮窗实例
let isNotificationShown = false
let isMouseInTray = false;
...
function createNotificationWindow() {
    ...
    let windowX = trayBounds.x;
    let windowY = trayBounds.y - windowHeight;
    ...
    // 加载页面
    if (isDevelopment) {
        notificationWindow.loadURL(`http://localhost:8080/#/notification`);
    } else {
        createProtocol('app');
        notificationWindow.loadURL(`app://./index.html#/notification`);
    }
    // --------------------------
    // 关键：通过 IPC 监听鼠标事件
    // --------------------------
    // 监听渲染进程发送的「鼠标进入悬浮窗」事件
    ipcMain.on('notification:mouse-enter', () => {
        isMouseInWindow = true;
        console.log('主进程：鼠标进入悬浮窗');
        clearTimeout(closeTimer); // 阻止关闭
    });

    // 监听渲染进程发送的「鼠标离开悬浮窗」事件
    ipcMain.on('notification:mouse-leave', () => {
        isMouseInWindow = false;
        console.log('主进程：鼠标离开悬浮窗');
        checkIfNeedClose(); // 检查是否关闭
    });
    ...
    notificationWindow.on('closed', () => {
        ...
        // 移除 IPC 事件监听（避免重复触发）
        ipcMain.removeAllListeners('notification:mouse-enter');
        ipcMain.removeAllListeners('notification:mouse-leave');
...
function checkIfNeedClose() {
    ...
    closeTimer = setTimeout(() => {
        // 只有鼠标既不在托盘也不在悬浮窗内，才关闭
        if (!isMouseInTray && !isMouseInWindow) {
    ...
    if (settings.showOnTray) {
        ...
        setTimeout(() => {
            if (!tray) {
                ...
                // tray.on('mouse-enter', () => {
                //     isMouseInTray = true;
                //     clearTimeout(closeTimer); // 清除可能的关闭计时器
                //     showNotificationWindow(); // 第一次显示悬浮窗
                // });

                // tray.on('mouse-leave', () => {
                //     isMouseInTray = false;
                //     checkIfNeedClose(); // 检查是否需要关闭
                // });
            }
    ...
    const onScreenShotEnd = (result) => {
        ...
        if (!result || !result.filePath) {
            ...
            if (isMainWindowFocusedWhenStartScreenshot) {
                ...
                const tms = windows.filter(win => win.webContents.id === screenShotWindowId);
                if (tms.length > 0) tms[0].show();
                screenShotWindowId = 0;
            }
            return;
        }

        // 使用已导入的 fs，读取截图文件并转 base64（回调写法，适配现有代码）
        fs.readFile(result.filePath, (readError, buffer) => {
            if (readError) {
                console.error('截图文件读取失败：', readError);
                return;
            }

            // 1. 二进制 buffer 转 base64 字符串（不带前缀）
            const base64Str = buffer.toString('base64');
            // 2. 拼接标准 dataURI 格式（带图片类型前缀，确保浏览器识别）
            const dataUri = `data:image/png;base64,${base64Str}`; // 截图若为 jpg，改为 data:image/jpeg;base64,

            // 3. 发送 base64 格式图片到渲染进程
            if (isMainWindowFocusedWhenStartScreenshot) {
                mainWindow.webContents.send('screenshots-ok', { dataUri }); // 传 dataUri 替代 filePath
                mainWindow.show();
                isMainWindowFocusedWhenStartScreenshot = false;
            } else if (screenShotWindowId) {
                const windows = BrowserWindow.getAllWindows();
                const tms = windows.filter(win => win.webContents.id === screenShotWindowId);
                if (tms.length > 0) {
                    tms[0].webContents.send('screenshots-ok', { dataUri });
                    tms[0].show();
                }
                screenShotWindowId = 0;
            }
