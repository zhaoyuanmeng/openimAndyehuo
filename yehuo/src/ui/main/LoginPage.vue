        ...
        <div class="login-container">
            ...
            <div v-if="loginType === 0" class="qrcode-login-container">
                ...
                <div v-if="loginStatus === 0" class="pending-scan">
                    ...
                    <a style="font-size: 15px; color: #b70006" target="_blank"
            ...
            <div v-else-if="loginType === 1" class="login-form-container">
                ...
                <button class="login-button" :disabled="mobile === '' || !password || password === ''"
                    ref="loginWithPasswordButton" @click="loginWithPassword">{{ loginStatus === 3 ? '数据同步中，可能需要数分钟...' :
                        '登录' }}</button>
                <ClipLoader v-if="loginStatus === 3" class="syncing" :color="'#b70006'" :height="'80px'"
                    :width="'80px'" />
            </div>
            <div v-else class="login-form-container">
                <!--            验证码登录-->
                <img class="logo" :src="require(`@/assets/images/icon.png`)" alt="">
                <p style="font-size: 22px;color: black;font-weight: 600;letter-spacing: 2px;">数城e家人</p>
                ...
                <button class="login-button" :disabled="mobile === '' || authCode === ''" ref="loginWithAuthCodeButton"
                    @click="loginWithAuthCode">{{ loginStatus === 3 ? '数据同步中，可能需要数分钟...' : '登录' }}</button>
            ...
            <!-- <div v-if="loginStatus === 0" class="switch-login-type-container">
                <p class="tip" @click="switchLoginType(loginType === 0 ? 1 : 0)">{{ loginType === 0 ? '使用密码/验证码登录' :
                    '扫码登录' }}</p>
            </div> -->
...
export default {
    ...
    data() {
        return {
            ...
            appToken: '',
            lastAppToken: '',
            loginType: 2, // 0 扫码登录，1 密码登录，2 验证码登录
