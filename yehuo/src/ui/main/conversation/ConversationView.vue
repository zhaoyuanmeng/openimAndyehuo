        ...
        <div v-else class="conversation-container">
            ...
            <div ref="conversationContentContainer" class="conversation-content-container"
                ...
                <SingleConversationInfoView
                    ...
                    v-v-on-click-outside="handleOutsideClick"
...
export default {
    ...
    methods: {
       handleOutsideClick(event) {
            // 判断点击目标是否是设置图标（或其内部元素）
            const settingIcon = this.$refs.setting;
            const isClickOnSettingIcon = settingIcon && (
                event.target === settingIcon || 
                settingIcon.contains(event.target)
            );
            // 如果不是点击设置图标，则关闭面板
            if (!isClickOnSettingIcon) {
...
/* 容器 hover 时，显示滑块和轨道（实际是让滑块变有色） */
.conversation-message-list:hover::-webkit-scrollbar-thumb {
