    ...
    <div ref="message-input-container" class="message-input-container">
        ...
        <section v-else-if="!sharedConversationState.showChannelMenu" style="display: flex; flex-direction: column;">
            <section class="input-action-container">
                ...
                <ul>
                    <li v-if="!inputOptions['disableEmoji']">
                        <i id="showEmoji" @click="toggleEmojiView" class="icon-Frame" title="发送表情" />
                    </li>
                    <li v-if="!inputOptions['disableFile']">
                        <i @click="pickFile" class="icon-Frame-1" title="发送文件" />
                        <input ref="fileInput" multiple @change="onPickFile($event)" class="icon-ion-ios-folder"
                            type="file" style="display: none">
                    </li>
                    <li v-if="!inputOptions['disableScreenShot'] && sharedMiscState.isElectron">
                        <div style="display: inline-block; text-align: center">
                            <i id="screenShot" @click="screenShot(false)" class="icon-Frame-11" />
                            <i class="icon-Frame-13"
                                style="font-size: 10px; color: #494849; padding-left: 5px;" />
                            <span @click="screenShot(true)" class="screen-shot-button" style="cursor: pointer;">隐藏当前窗口截图</span>
                        </div>
                    </li>
                    <li v-if="!inputOptions['disableHistory'] && sharedMiscState.isElectron">
                        <i id="messageHistory" @click="showMessageHistory" class="icon-ion-android-chat" title="历史消息" />
                    </li>
                    <li v-if="enablePtt">
            ...
            <div @keydown.enter="send($event)" ref="input" class="input" @paste="handlePaste" draggable="false"
                title="" autofocus @input="onInput" @contextmenu.prevent="$refs.menu.open($event)"
    ...
    mounted() {
        ...
        if (isElectron()) {
            ipcRenderer.on('screenshots-ok', (event, args) => {
                ...
                if (args.dataUri) {
                    setTimeout(() => {
                        document.execCommand('insertImage', false, args.dataUri);
...
>>>.emoji-picker {
    ...
    --ep-color-active: #B70006 !important;
}
</style>
