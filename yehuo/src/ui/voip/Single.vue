        ...
        <div v-if="session && !(session.screenSharing && session.rcStatus === 5)" class="container" style="background: #292929">
            <section class="full-height full-width">
                ...
                <div class="content-container" v-if="audioOnly">
                    ...
                    <div class="remote-media-container" :style=" {backgroundImage: `url(${participantUserInfo?.portrait || ''})`}">
            ...
            <!--actions-->
            <footer style="z-index: 999 !important;">
                <!--incoming-->
...
.remote-media-container {
    ...
    position: relative; /* 关键：让伪元素相对于容器定位 */
    overflow: hidden; /* 避免模糊边缘溢出 */
}

/* 高斯模糊背景图（通过伪元素实现，不影响子内容） */
.remote-media-container::before {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    /* 继承父元素的背景图（即 participantUserInfo.portrait） */
...
.remote-media-container::after {
    ...
    width: 100%;
    height: 100%;
    background-color: rgba(0, 0, 0, 0.4); /* 黑色半透明（透明度可调整） */
    z-index: 1; /* 位于模糊背景上方，子内容下方 */
}

/* 确保子元素（头像、文字、视频）在蒙层上方显示 */
.remote-media-container > img,
.remote-media-container > video,
.remote-media-container > p,
.remote-media-container > div {
    position: relative;
    z-index: 2; /* 高于蒙层和背景 */
}
