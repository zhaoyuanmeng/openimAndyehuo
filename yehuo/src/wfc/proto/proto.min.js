...
class WfcImpl {
    ...
    onConnectionChanged(status) {
        ...
        if (status === ConnectionStatus.ConnectionStatusConnected) {
            if (self.lastActiveTime === 0) {
                ...
                console.log('to load default data estimate duration', estimatedTime);
                setTimeout(() => {
                    self.connectionStatus = status;
                    self.lastActiveTime = Math.ceil(new Date().getTime() / 1000);
                    console.log('emit ConnectionStatusConnected after', estimatedTime);
    ...
    onConnectToServer(host, ip, port) {
        console.log('connecting to server', host, ip, port);
    ...
    onReceiveMessage(messages, hasMore) {
        ...
        msgs.forEach(m => {
            ...
            if (msg && msg.messageContent instanceof MarkUnreadMessageContent && msg.from === self.userId) {
    ...
    onDomainInfoUpdate(domInfoStr) {
        ...
        let domainInfo = Object.assign(new DomainInfo(), JSON.parse(domInfoStr));
    ...
    onUserInfoUpdate(userIds) {
        ...
        console.log('onuserInfoUpdate', userInfos.length);
    ...
    onTrafficData(send, recv) {
        // console.log('onTrafficData:', send, recv);
    }

    onErrorEvent(errType, errMsg) {
        console.log('onErrorEvent', errType, errMsg);
    }

    init(args = []) {
        protoProxy.init();
        // if(process.platform === 'win32'){
        //     protoProxy.invoke('setAppDataPath', process.cwd());
        // }
    ...
    connect(userId, token) {
        console.log('connect', userId, token)
        if (!self.userId) {
            self.userId = userId;
            let status = this.getConnectionStatus();
            if (status !== ConnectionStatus.ConnectionStatusConnecting && status !== ConnectionStatus.ConnectionStatusLogout) {
    ...
    setTimeOffset(offset) {
        protoProxy.invoke('setTimeOffset', offset);
    }
    ...
    async searchUserEx(domainId, keyword, searchType, page, successCB, failCB) {
        protoProxy.invokeAsync('searchUserEx', domainId, keyword, searchType, page, (result) => {
            let userListStr = JSON.parse(result);
            let userList = [];
            if (userListStr && userListStr.length > 0) {
                userListStr.forEach(u => {
    ...
    clearFriendRequest(direction, beforeTime) {
        protoProxy.invoke('clearFriendRequest', !direction, beforeTime + '');
    }

    deleteFriendRequest(userId, direction) {
        protoProxy.invoke('deleteFriendRequest', userId, !direction);
    }
    ...
    getFriendExtra(userId) {
        return protoProxy.invoke('getFriendExtra', userId);
    }
    ...
    getFirstUnreadMessageId(conversation) {
        return protoProxy.invoke('getFirstUnreadMessageId', JSON.stringify(conversation));
    }
    ...
    searchMessageByTypesAsync(conversation, keyword, contentTypes, desc, limit, offset, withUser = '', resultCB = null) {
        ...
        protoProxy.invokeAsync('searchMessageByTypesAsync', JSON.stringify(conversation), keyword, contentTypes, desc, limit, offset, withUser, result => {
            let msgs = JSON.parse(result);
            let matchMsgs = [];
            if (msgs && msgs.length > 0) {
                msgs.forEach(m => {
            ...
            this._uploadFile(insertedMsg, file, message.messageContent.mediaType, (uploaded, total) => {
                ...
                protoProxy.invokeAsync('sendSavedMessage', insertedMsg.messageId, 0, (messageUid, timestamp) => {
                    ...
                    if (insertedMsg.conversation.type !== ConversationType.ChatRoom) {
                        this.eventEmitter.emit(EventType.MessageStatusUpdate, insertedMsg);
                    }
    ...
    // 更新了原始消息的内容
    recallMessage(messageUid, successCB, failCB) {
    ...
    async clearMessagesKeepLatest(conversation, count) {
        protoProxy.invoke('clearMessagesKeepLatest', JSON.stringify(conversation), count);
    }
    ...
    getMessageCount(conversation) {
        var count = protoProxy.invoke('getMessageCount', JSON.stringify(conversation));
        return count;
    }

    getConversationMessageCount(types, lines) {
        var count = protoProxy.invoke('getConversationMessageCount', types, lines);
        return count;
    }
    ...
    // 处理 dataUri 格式（若传入的是含前缀的 base64 字符串）
    if (typeof fileOrData === 'string' && fileOrData.indexOf("base64,") >= 0) {
        fileOrData = fileOrData.substring(fileOrData.indexOf(',') + 1);
    }

    // 分支1：TCP短连接 或 支持大文件上传（走 _uploadFile 逻辑）
    if (this.isTcpShortLink() || this.isSupportBigFilesUpload()) {
        let file;
        // 若 fileOrData 是 base64 字符串，转换为 Blob 对象
        ...
        // 若 fileOrData 是 File/Blob 对象，直接使用
        else {
            file = fileOrData;
        }
        this._uploadFile(null, file, mediaType, progressCB, successCB, failCB);
    } 
    // 分支2：非 TCP短连接 且 不支持大文件上传（走 protoProxy 逻辑）
    else {
        // 处理 浏览器端 File 对象（无 path 属性，需转为 base64）
        if (fileOrData instanceof File) {
            fileOrData = await new Promise((resolve, reject) => {
                const reader = new FileReader();
                reader.readAsDataURL(fileOrData); // 读取为 dataURI 格式
                reader.onload = () => {
                    // 截取 base64 部分（去除 "data:xxx;base64," 前缀）
                    const base64Str = reader.result.split(',')[1];
                    resolve(base64Str);
                };
                reader.onerror = (error) => {
                    console.error('File 对象转 base64 失败：', error);
                    reject(error);
                    failCB && failCB(-2); // 自定义错误码，标识转换失败
                };
            });
        } 
        // 处理 Node.js 环境文件对象（有 path 属性，通过 fs 读为 base64）
        else if (fileOrData.path) {
            try {
                fileOrData = await fs.readFile(fileOrData.path, { encoding: 'base64' });
            } catch (error) {
                console.error('读取文件为 base64 失败：', error);
                failCB && failCB(-3); // 自定义错误码，标识文件读取失败
                return;
            }
        } 
        // 若既不是 File 对象也无 path 属性，需确保已是 base64 字符串（否则后续会报错）
        else if (typeof fileOrData !== 'string') {
            console.error('fileOrData 格式错误：需为 File 对象、带 path 的文件对象或 base64 字符串');
            failCB && failCB(-4); // 自定义错误码，标识参数格式错误
            return;
        }

        // 调用 protoProxy，此时 fileOrData 已确保为 base64 字符串（符合参数类型要求）
        protoProxy.invokeAsync(
            ...
            (errorCode) => {
                console.error('uploadMedia 调用失败，错误码：', errorCode);
                failCB && failCB(errorCode);
            },
            (current, total) => {
    ...
    getDomainInfo(domainId, refresh) {
        let domainInfoStr = protoProxy.invoke('getDomainInfo', domainId, refresh);
        let domainInfo;
        if (domainInfoStr === '') {
            domainInfo = new DomainInfo();
            domainInfo.domainId = domainId;
        } else {
            domainInfo = Object.assign(new DomainInfo(), JSON.parse(domainInfoStr));
        }
        return domainInfo;
    }

    loadRemoteDomains(successCB, failCB) {
        protoProxy.invokeAsync('loadRemoteDomains', (domainInfosStr) => {
            let domainInfos = []
    ...
    _setupEventListeners() {
        ...
        protoProxy.setProtoEventListener('connectedToServer', (host, ip, port, mainNw) => {
        ...
        protoProxy.setProtoEventListener('domainInfoUpdate', (domainInfoStr) => {
        ...
        protoProxy.setProtoEventListener('trafficDataEvent', (send, recv) => {
            self.onTrafficData(send, recv);
        });
        protoProxy.setProtoEventListener('errorEventCallback', (errType, errMsg) => {
            self.onErrorEvent(errType, errMsg);
        });
    }
    ...
    _preloadDefaultData() {
        ...
        for (let i = 0; i < uids.length / 2000; i++) {
            let infos = self.getUserInfos(uids.slice(2000 * i, (i + 1) * 2000), '');
        ...
        let estimatedTime = 0;
        // 超过一周没有活跃，就预加载数据
        if (new Date().getTime() / 1000 - self.lastActiveTime > 7 * 24 * 60 * 60) {
            // 每 2000 人 4 秒
    ...
    _preloadGroupMemberUserInfos(memberIds) {
        ...
        let toLoadIds = memberIds.filter(uid => friendIds.indexOf(uid) === -1)
        for (let i = 0; i < toLoadIds.length / 2000; i++) {
            self.getUserInfos(memberIds.slice(2000 * i, (i + 1) * 2000), '');
            console.log('to preload groupMember userInfos', memberIds.slice(2000 * i, (i + 1) * 2000))
        }
    }
